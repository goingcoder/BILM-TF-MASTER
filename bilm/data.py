...
class UnicodeCharsVocabulary(Vocabulary): #UnicodeCharsVocabulary类 继承 Vocabulary类
    ...
    def _convert_word_to_char_ids(self, word):
        ...
        word_encoded = word.encode('utf-8', 'ignore')[:(self.max_word_length-2)] # ignore:忽略其中有异常的编码，仅显示有效的编码
        code[0] = self.bow_char
        for k, chr_id in enumerate(word_encoded, start=1): #start=1 起始下标
            code[k] = chr_id
    ...
    def encode_chars(self, sentence, reverse=False, split=True):
        ...
        if reverse:
            return np.vstack([self.eos_chars] + chars_ids + [self.bos_chars]) # np.vstack 垂直，第1维 维度相加； np.hstack 水平 第2维 维度相加
        else:
            return np.vstack([self.bos_chars] + chars_ids + [self.eos_chars]) # 返回numpy数组 (sen_len,max_word_len)
...
class LMDataset(object):
    ...
    def __init__(self, filepattern, vocab, reverse=False, test=False,
        ...
        '''
        self._vocab = vocab
        self._all_shards = glob.glob(filepattern) # filepattern里会有通配符，返回filepattern代表的文件名 列表
        ...
        self._use_char_inputs = hasattr(vocab, 'encode_chars') # hasattr：判断某个对象是否有某个属性/方法 返回True/False
    ...
    def _choose_random_shard(self):
        ...
        shard_name = self._shards_to_choose.pop() # pop(index=-1) 删除list中下标为index的元素，并返回元素值。默认删除最后一个元素。
        return shard_name
    ...
    def _load_shard(self, shard_name):
        ...
        ids = [self.vocab.encode(sentence, self._reverse) # 写错了！！应该是self._vocab
               for sentence in sentences] # list长度为sent_num，每个元素是numpy数组，表示这个句子的每个word的id
        if self._use_char_inputs:
            chars_ids = [self.vocab.encode_chars(sentence, self._reverse) # 写错了！！应该是self._vocav
                     for sentence in sentences] # list长度为sent_num，每个元素是二维numpy数组
        else:
            chars_ids = [None] * len(ids)

        print('Loaded %d sentences.' % len(ids))
        print('Finished loading')
        return list(zip(ids, chars_ids)) # list长度为sent_num，每个元素是(id_list,char_id_list)
