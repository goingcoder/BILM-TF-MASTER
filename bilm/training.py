...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        self.token_ids = tf.placeholder(DTYPE_INT,
                               shape=(batch_size, unroll_steps), # unroll_steps表示句子最长有多少个word
                               name='token_ids')
        # the word embeddings
        with tf.device("/cpu:0"):
            ...
            self.embedding = tf.nn.embedding_lookup(self.embedding_weights,
                                                self.token_ids) # (batch_size,unroll_steps,projection_dim)
    ...
    def _build_word_char_embeddings(self):
        ...
        # the convolutions
        def make_convolutions(inp, reuse): # inp (batch_size,unroll_steps,max_chars,embed_dim)
            with tf.variable_scope('CNN', reuse=reuse) as scope:
                ...
                for i, (width, num) in enumerate(filters):
                    ...
                    conv = tf.nn.conv2d(
                            inp, w,
                            strides=[1, 1, 1, 1],
                            padding="VALID") + b  # (batch_size,unroll_steps,max_chars-width+1,num)
                    # now max pool
                    conv = tf.nn.max_pool(
                            conv, [1, 1, max_chars-width+1, 1],
                            [1, 1, 1, 1], 'VALID')  # (batch_size,unroll_steps,1,num) 每一个filter取最大的那个
            ...
            return tf.concat(convolutions, 2) # (batch_size,unroll_steps,n_filters)

        # for first model, this is False, for others it's True
        reuse = tf.get_variable_scope().reuse
        embedding = make_convolutions(self.char_embedding, reuse) # (batch_size,unroll_steps,n_filters)

        self.token_embedding_layers = [embedding]  # 卷积后，第1层highway后，第2层highway后，projection后

        if self.bidirectional:
            # re-use the CNN weights from forward pass
            embedding_reverse = make_convolutions(
                self.char_embedding_reverse, True) # (batch_size,unroll_steps,n_filters)
        ...
        if use_highway or use_proj:
            embedding = tf.reshape(embedding, [-1, n_filters]) # (batch_size*unroll_steps,n_filters)
            if self.bidirectional:
                embedding_reverse = tf.reshape(embedding_reverse,
                    [-1, n_filters]) # (batch_size*unroll_steps,n_filters)
        ...
        if use_highway:
            ...
            for i in range(n_highway):
                ...
                embedding = high(embedding, W_carry, b_carry,
                                 W_transform, b_transform)  # (batch_size*unroll_steps,n_filters)
                if self.bidirectional:
        ...
        if use_proj:
            embedding = tf.matmul(embedding, W_proj_cnn) + b_proj_cnn # (batch_size*unroll_steps,projection_dim)
            if self.bidirectional:
    ...
    def _build(self):
        ...
        for lstm_num, lstm_input in enumerate(lstm_inputs):
            ...
            if n_lstm_layers > 1:
                lstm_cell = tf.nn.rnn_cell.MultiRNNCell(lstm_cells) # tf.nn.rnn_cell.MultiRNNCell用来实现多层RNN，第一层的输出h是第二层的输入，第二层的输出h是第三层的输入...
            else:
                ...
                if self.bidirectional:
                    with tf.variable_scope('RNN_%s' % lstm_num):
                        _lstm_output_unpacked, final_state = tf.nn.static_rnn(
                            lstm_cell,
                            tf.unstack(lstm_input, axis=1), # tf.stack矩阵拼接 tf.unstack矩阵分解 
                            initial_state=self.init_lstm_state[-1])
                else:
                    _lstm_output_unpacked, final_state = tf.nn.static_rnn( # 返回(output,state)
                        lstm_cell,
                        tf.unstack(lstm_input, axis=1), # (batch_size,unroll_steps,projection_dim) -> 长度为unroll_steps的list，每个元素shape(batch_size,projection_dim)
                        initial_state=self.init_lstm_state[-1])
                self.final_lstm_state.append(final_state)

            # (batch_size * unroll_steps, 512)
            lstm_output_flat = tf.reshape(
                tf.stack(_lstm_output_unpacked, axis=1), [-1, projection_dim]) # (batch_size,lstm_dim*T) -> (batch_size*T,projection_dim)
            if self.is_training:
                # add dropout to output
                lstm_output_flat = tf.nn.dropout(lstm_output_flat,
                    keep_prob)
            tf.add_to_collection('lstm_output_embeddings',
                _lstm_output_unpacked) # tf.add_to_collection 把变量放入一个集合
    ...
    def _build_loss(self, lstm_outputs):
        ...
        for id_placeholder, lstm_output_flat in zip(next_ids, lstm_outputs):
            ...
            next_token_id_flat = tf.reshape(id_placeholder, [-1, 1]) # (batch_size,unroll_steps) -> (batch_size*unroll_steps,1)

            with tf.control_dependencies([lstm_output_flat]):
                if self.is_training and self.sample_softmax:
                    losses = tf.nn.sampled_softmax_loss( # importance sampling
                                   ...
                                   num_true=1) # 返回(batch_size*unroll_steps)

                else:
                    # get the full softmax loss
                    output_scores = tf.matmul(
                        lstm_output_flat,  # (batch_size*unroll_steps,projection_dim)
                        tf.transpose(self.softmax_W)  # (softmax_dim,n_tokens_vocab)
                    ) + self.softmax_b  # (batch_size*unroll_steps,n_tokens_vocab)
                    # NOTE: tf.nn.sparse_softmax_cross_entropy_with_logits
                    #   expects unnormalized output since it performs the
                    #   softmax internally
                    losses = tf.nn.sparse_softmax_cross_entropy_with_logits(
                        logits=output_scores,  # (batch_size*unroll_steps,n_tokens_vocab)
                        labels=tf.squeeze(next_token_id_flat, squeeze_dims=[1]) # (batch_size*unroll_steps)
                    ) # 返回(batch_size*unroll_steps)
...
def average_gradients(tower_grads, batch_size, options):
    ...
    for grad_and_vars in zip(*tower_grads):
        ...
        if isinstance(g0, tf.IndexedSlices):  # 没看懂！！！
            ...
            for g, v in grad_and_vars:
                ...
                expanded_g = tf.expand_dims(g, 0) # (1,1)
                # Append on a 'tower' dimension which we will average over 
                grads.append(expanded_g)

            # Average over the 'tower' dimension.
            grad = tf.concat(grads, 0) # (gpuN,1)
            grad = tf.reduce_mean(grad, 0) # (1)
    ...
    return average_grads  # 长度为var_num的list，每个元素为元组(grad,v)
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    if restart_ckpt_file is None: # ckpt--checkpoint
        with open(os.path.join(tf_save_dir, 'options.json'), 'w') as fout:
            fout.write(json.dumps(options)) # json.dumps:把dict转化为str json.loads:把str转化为dict

    with tf.device('/cpu:0'):
        global_step = tf.get_variable(
            'global_step', [],
            initializer=tf.constant_initializer(0), trainable=False) # 为什么[]？？？

        # set up the optimizer
        lr = options.get('learning_rate', 0.2) # dict.get(key,default=None) default:如果指定键的值不存在时，返回该默认值。
        ...
        for k in range(n_gpus):
            with tf.device('/gpu:%d' % k):
                with tf.variable_scope('lm', reuse=k > 0): # k==0时是False k==1,2,...时是True 为什么！！
                    ...
                    grads = opt.compute_gradients(
                        loss * options['unroll_steps'], # loss是每个word的，*options['unroll_steps']后，表示每个句子（样本）的
